...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid") || 0 == commandlineArguments.count("freq")) {
    ...
    float time =0.0f;
    auto atFrequency{[&VERBOSE, &behavior, &od4, &time,&FREQ]() -> bool
      { 

        behavior.step(time);
        time=time + (1.0f/FREQ);

        auto wheelSpeedRequestLeft = behavior.getWheelSpeedRequestLeft();
        auto wheelSpeedRequestRight = behavior.getWheelSpeedRequestRight();

        cluon::data::TimeStamp sampleTime = cluon::time::now();
        od4.send(wheelSpeedRequestLeft, sampleTime, 0);
        od4.send(wheelSpeedRequestRight, sampleTime, 1);
